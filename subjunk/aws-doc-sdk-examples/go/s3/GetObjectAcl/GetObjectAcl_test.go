...
func TestGetObjectAcl(t *testing.T) {
    ...
    if globalConfig.Bucket == "" || globalConfig.Key == "" {
        ...
        bucketCreated = true
        t.Log("Uploaded " + globalConfig.Key + " to " + globalConfig.Bucket)
    }

    result, err := GetObjectACL(sess, &globalConfig.Bucket, &globalConfig.Key)
    if err != nil {
        t.Fatal(err)
    }

    t.Log("Owner:", *result.Owner.DisplayName)
    t.Log("")
    t.Log("Grants")
