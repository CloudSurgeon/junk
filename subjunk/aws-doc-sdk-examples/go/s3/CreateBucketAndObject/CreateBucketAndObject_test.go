...
func TestCreateBucket(t *testing.T) {
    ...
    if globalConfig.Key == "" {
        globalConfig.Key = "TestFile.txt"
    }

    err = CreateBucketAndObject(sess, &globalConfig.Bucket, &globalConfig.Key)
    if err != nil {
        t.Fatal(err)
    }

    t.Log("Created bucket " + globalConfig.Bucket + " with object " + globalConfig.Key)
