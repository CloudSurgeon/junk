...
func populateConfiguration(t *testing.T) error {
    ...
    t.Log("Content: " + globalConfig.Content)
...
func TestGeneratePresignedURL(t *testing.T) {
    ...
    if globalConfig.Bucket == "" || globalConfig.Key == "" || globalConfig.Content == "" {
        id := uuid.New()
        globalConfig.Bucket = "test-bucket-" + id.String()
        globalConfig.Key = "test.txt"
        globalConfig.Content = "This is a test"
    ...
    urlStr, err := GetPresignedURL(sess, &globalConfig.Bucket, &globalConfig.Key, &globalConfig.Content)
    if err != nil {
        t.Fatal(err)
    }
